...
option_mapping = {
    'AYUDAS'        : ["Estado SAT", "Fecha Consulta", "Version", "Fecha Emision", "Fecha Timbrado", "Serie", "Folio", "UUID", "Tipo Comprobante", "Tipo", "Metodo Pago", "Forma Pago", "Subtotal","Total", "Moneda", "Tipo Relacion", "UUID Relacion", "CP Emisor", "RFC Emisor", "Nombre Emisor", "Regimen Fiscal Emisor", "RFC Receptor", "Nombre Receptor", "CP Receptor", "Regimen Fiscal Receptor", "Uso CFDI", "Lista de Conceptos", "Lista Unidad", "Lista Clave de Unidad", "Lista Clave de Producto", "Total Retenciones Locales", "Total Imp Retenidos", "Total Traslados", "Total Imp Trasladados", "Periodicidad_Global", "Meses_Global", "Año_Global", "Sello", "No Certificado", "Certificado", "Leyenda","Addenda", "Archivo XML", "Num. Cuenta Contable", "Nombre Cuenta Contable", "Número CRI/COG", "Nombre CRI/COG", "Precisiones"],
            ...
            if key in row:
                try:
                    ...
                    if node.tag.endswith('ImpuestosLocales'):
                        ...
                        row['Imp Local Retenido']          = string_to_double(node.attrib.get('ImpLocRetenido'              , ''))
                        row['Tasa Imp Local Retenido']     = string_to_double(node.attrib.get('TasadeRetencion'             , ''))
                        row['Importe Imp Local Retenido']  = string_to_double(node.attrib.get('Importe'                     , ''))
                    ...
                    if node.tag.endswith('Conceptos'):
                        ...
                        row['Lista Clave de Producto']  = json.dumps(prod_list, ensure_ascii=False)
                        row['Lista Clave de Unidad']    = json.dumps(key_unit_list, ensure_ascii=False)
                        row['Lista Unidad']             = json.dumps(unit_list, ensure_ascii=False)
                    ...
                    if node.tag.endswith('Retenciones'):
                        ...
                        for transfer in transfers:
                            if   transfer.get('Impuesto') == "001":
                                row['Retención ISR']             = string_to_double(node.attrib.get('Importe', ''))

                            elif transfer.get('Impuesto') == "002":
                                row['Retención IVA']             = string_to_double(node.attrib.get('Importe', ''))

                            elif transfer.get('Impuesto') == "003":
                                row['Retención IEPS']             = string_to_double(node.attrib.get('Importe', ''))
                    ...
                    if node.tag.endswith('Traslado'):
                        ...
                        for transfer in transfers:
                            if   transfer.get('Impuesto') == "001":
                                row['ISR']      = string_to_double(node.attrib.get('Importe', ''))

                            elif transfer.get('Impuesto') == "002":
                                row['IVA 16%']  = string_to_double(node.attrib.get('Importe', ''))

                            elif transfer.get('Impuesto') == "003":
                                row['IEPS']     = string_to_double(node.attrib.get('Importe', ''))
